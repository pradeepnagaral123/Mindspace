  ...
  return (
    <Card className="flex flex-col p-6">
      <div className="mb-5 flex items-center justify-between">
        <h3 className="text-base font-bold text-ink">Upcoming Events</h3>
        <Link
          to="/events"
      ...
      <div className="space-y-3.5">
        {events.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="rounded-xl border border-line/80 bg-cream/50 p-3.5"
            >
              <div className="flex items-center gap-3">
                <span
                  className={`flex h-11 w-11 shrink-0 flex-col items-center justify-center rounded-xl ${event.color}`}
                >
              ...
              <div className="mt-3 flex items-center justify-between">
                <span
                  className={`rounded-full px-2.5 py-1 text-[10.5px] font-semibold ${event.color}`}
                >
                  {event.type}
                </span>
                <button
